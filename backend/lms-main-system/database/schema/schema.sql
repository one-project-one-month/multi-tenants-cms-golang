...
CREATE
OR REPLACE FUNCTION prevent_student_tenant_membership()
    RETURNS TRIGGER AS $$
BEGIN
    IF
EXISTS (
...
CREATE
OR REPLACE FUNCTION auto_assign_student_to_namespace_consumer()
    RETURNS TRIGGER AS $$
DECLARE
...
IF
user_tenant_id IS NOT NULL THEN
SELECT namespace
INTO student_namespace
FROM Tenants
...
IF
student_namespace IS NOT NULL THEN
