...
func (g *Gateway) Start() error {
	...
	if err := assignmentpb.RegisterAssignmentServiceHandlerFromEndpoint(ctx, gwMux, g.grpcAddr, opts); err != nil {
		return fmt.Errorf("failed to register assignment service gateway: %w", err)
	}

	if err := coursepb.RegisterCourseServiceHandlerFromEndpoint(ctx, gwMux, g.grpcAddr, opts); err != nil {
		return fmt.Errorf("failed to register course service gateway: %w", err)
	}

	if err := modulepb.RegisterModuleServiceHandlerFromEndpoint(ctx, gwMux, g.grpcAddr, opts); err != nil {
		return fmt.Errorf("failed to register module service gateway: %w", err)
	}

	if err := tenantpb.RegisterTenantServiceHandlerFromEndpoint(ctx, gwMux, g.grpcAddr, opts); err != nil {
		return fmt.Errorf("failed to register tenant service gateway: %w", err)
	}
...
func (g *Gateway) metadataAnnotator(ctx context.Context, req *http.Request) metadata.MD {
	md := metadata.MD{}
	md["x-request-id"] = []string{
		req.Header.Get("X-Request-Id"),
	}
	md["x-tenant-id"] = []string{
		req.Header.Get("X-Tenant-Id"),
	}
	md["user-agent"] = []string{
